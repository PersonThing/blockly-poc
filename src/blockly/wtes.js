...
const wtes = {
    ...
    // wtes defined by define_wte blocks and called by call_wte blocks
    wteDefinitions: {},
    ...
    call_wte: (name, contextOverrides) => {
        ...
        if (wteFunc) {
            ...
            return wtes.logAndReturn(`wte:${name}`, null, wteFunc(context));
        }
        console.error(`No WTE defined with name ${name}`);
        return wtes.logAndReturn('wte_error', { name }, `No WTE defined with name ${name}`, null);
    },
    ...
    tier_intersection: (params) => {
        ...
        for (const t of thresholds) {
            ...
            wtes.logAndReturn(`tier_check:${min}-${max}`, { input, tier: t }, minCheck && maxCheck);
    ...
    */
    ...
    array_math: (mathType, list) => {
        const result = arrayMath(mathType, list);
